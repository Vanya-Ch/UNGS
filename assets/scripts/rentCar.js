...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const carToDriver = {
        ...
        'Toyota Hilux  ВІ 0019 ВМ': 'Олександр ПАТОЬОМА',
...
document.addEventListener('DOMContentLoaded', function () {
    fetch('/rentCar')
        .then(response => response.json())
        .then(data => {
            displayBookingsByDriver(data);
        })
        .catch(error => {
            console.error('Помилка отримання даних про бронювання:', error);
        });
})

function displayBookingsByDriver(bookings) {
    const driversContainer = document.getElementById('driversContainer');
    driversContainer.innerHTML = '';  // Очищаємо контейнер перед заповненням

    // Групуємо бронювання за водіями
    const bookingsByDriver = bookings.reduce((acc, booking) => {
    ...
    Object.keys(bookingsByDriver).forEach(driver => {
        ...
        `;
        driverHeaderContainer.appendChild(svgIcon);

        // Створюємо заголовок з ім'ям водія
        ...
        // Отримуємо сьогоднішню і завтрашню дати
...
/* document.addEventListener('DOMContentLoaded', async function () {
    try {
        ...
        const bookingsByDriver = drivers.reduce((acc, driver) => {
            acc[driver._id] = bookings.filter(booking => booking.driverId === driver._id);
            return acc;
        }, {});

        console.log(bookings, drivers);
